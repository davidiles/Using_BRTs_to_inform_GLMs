...
We first analyze the dataset with boosted regression trees, using the following code.  We also plot variable importance.

```{r fit_brt, cache = TRUE, results = "hide",fig.width=6, fig.height=12}
...
We conducted 150 simulations.  For each simulation, we stored the Root Mean Squared Error and correlation between model predictions and the 'true' response surface (y). 
...
The Bayesian model resulted in lower RMSE for `r 100*round(mean(results$RMSE_inla < results$RMSE_brt),2)`% of simulations.  

Additionally, predictions from the Bayesian model had a higher correlation with the true response surface in `r 100*round(mean(results$cor_inla > results$cor_brt),2)`% of simulations.

# Conclusions

This analysis suggests that a Bayesian spatial model outperforms a machine learning model 